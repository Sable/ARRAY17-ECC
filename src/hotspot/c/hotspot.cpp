		...
		for (c = 0; c < col; c++) {
			...
			if ( (r == 0) && (c == 0) ) {
				delta = (step / Cap) * (power[0] +
						...
						(temp[col] - temp[0]) / Ry +
						(amb_temp - temp[0]) / Rz);
				//printf("1 = %g, 2 = %g, 3 = %g\n",temp[1]-temp[0],temp[col]-temp[0],amb_temp-temp[0]);
				//printf("4 = %g, 5 = %g, 6 = %g, 7 = %g\n",step/Cap,power[0],(temp[1] - temp[0]) / Rx,(temp[col] - temp[0]) / Ry,(amb_temp - temp[0]) / Rz);
			}	/*	Corner 2	*/
			else if ((r == 0) && (c == col-1)) {
...
void compute_tran_temp(double *result, int num_iterations, double *temp, double *power, int row, int col) 
{
     ...
     for (int i = 0; i < num_iterations ; i++)
	{
		...
		#endif
		//printf("row = %d, col = %d, Cap = %g, Rx = %g, Ry = %g, Rz = %g, step = %g\n",row,col,Cap,Rx,Ry,Rz,step);
